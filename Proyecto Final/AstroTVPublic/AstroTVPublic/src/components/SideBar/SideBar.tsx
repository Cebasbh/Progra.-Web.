        ...
        <div className="container-fluid">
            <h5 className="TextBox">Canales que sigues</h5>
            {
                streamsfollowed.map((stream : Stream, index: number) => {
                    return(
                        <StreamBox key={`followed-${stream.id}-${index}`} doViewersDivision={props.doViewersDivision} stream = {stream}></StreamBox>
                    )
                })
            }
            <h5 className="TextBox">Canales que podrían interesarte</h5>
            {
                props.streams.map((stream : Stream, index: number) => {
                    return(
                        <StreamBox key={`recommended-${stream.id}-${index}`} doViewersDivision={props.doViewersDivision} stream = {stream}></StreamBox>
                    )
                })
            }
        </div>
