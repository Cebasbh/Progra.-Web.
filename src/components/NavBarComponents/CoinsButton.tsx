						...
						<>
							...
							<li className="d-flex  dropdown-item-text align-items-center justify-content-between w-100">
								<div className="d-flex flex-column text-start mx-2">
									<h5 className="TextBox">{pack.name}</h5>
									<h6 className="TextBox">{pack.value} <i className="bi bi-star-fill ministars"></i></h6>
								</div>
								<div className="d-flex flex-column text-end align-items-start mx-2">
									<button onClick={() => navigate("/payment", {state : {pack}})} className="mx-2 page-button border-0 buybutton">PEN {pack.finalprice}</button>
									<h6 className="Discount mx-3">{pack.discount}% de descuento</h6>
								</div>
							</li>
						</>
                    )})
